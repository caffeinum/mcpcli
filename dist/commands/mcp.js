...
export default class Mcp extends Command {
    ...
    static description = 'Connect to MCP daemon and run tools';
    static examples = [
    ...
    async ensureServerRunning() {
        ...
        let inferredTool = args.tool;
        let inferredToolArgs = (Array.isArray(args.toolArgs) ? args.toolArgs : (args.toolArgs ? [args.toolArgs] : []));
    ...
    async callToolByName(serverId, toolName, toolArgs, verbose = false) {
        try {
    ...
    async runInteractiveToolRunner(serverId, verbose = false) {
                ...
                switch (command.toLowerCase()) {
                    ...
                    case 'call': {
                        ...
                        if (tool.inputSchema?.properties) {
                            ...
                            if (toolArgs === null)
                                continue; // Skip execution if required param missing
                        }
                        try {
                            if (verbose) {
