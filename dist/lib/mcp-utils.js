export function isPathish(token) {
 ...
 */
export function splitServerArgsAndTool(flagArgs = [], explicitTool, explicitToolArgs = []) {
...
export function coerceByType(raw, schema) {
...
export function parseCliArgsForTool(tool, argv) {
    ...
    for (let i = 0; i < pos.length && i < keys.length; i++) {
        const k = keys[i];
        if (out[k] === undefined)
            out[k] = coerceByType(pos[i], props[k]);
    }
    // Validate required
